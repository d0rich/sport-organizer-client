...
export default {
  ...
  computed:{
    ...mapGetters(['user_profile', 'get_auth_user','tMode']),
    groups(){
      if(this.tMode) return this.user_profile.TrainerIn
      else return this.user_profile.TraineeIn
    }
  },
  watch:{
    tMode(){
  ...
  methods: {
    ...
    getEvents(){
      ...
      if (this.groupsReq.length === 0){
        if (this.tMode)
          this.user_profile.TrainerIn.forEach( group => {
